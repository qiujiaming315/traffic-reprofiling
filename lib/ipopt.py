...
def formulate_fifo(path_matrix, flow_profile, objective, weight, no_shaping=False):
    """
    ...
    :param objective: the objective function.
    :param weight: the bandwidth weight profile.
    :param no_shaping: Whether traffic shaping is applied to modify the traffic profiles.
    ...
    if no_shaping:
        for flow_idx in range(num_flow):
            delete_component(model, r"D[0-9]+_ct[0-9]+")
            setattr(model, f"D{flow_idx}_ct", pyo.Constraint(
                expr=(getattr(model, f"D{flow_idx}") == 0)
            ))
    ...
    def two_slope_solver(order):
        ...
        for link_idx in range(num_link):
            ...
            if no_shaping:
                burst_total = np.sum(flow_profile[link_mask, 1])
                setattr(model, f"C{link_idx}_ct", pyo.Constraint(expr=(getattr(model, f"C{link_idx}") >= burst_total / (
                            getattr(model, f"T{link_idx}")))))
            else:
                ...
                for ct_idx, flow_idx in enumerate(link_order):
                    ...
                    base_expr += (sum(
                        flow_profile[f_idx, 1] / getattr(model, f"D{f_idx}") for f_idx in
                        link_order[ct_idx:]) + base_r) * (new_ddl - base_ddl)
                    setattr(model, f"C{link_idx}_ct{ct_idx}", pyo.Constraint(
                        expr=(getattr(model, f"C{link_idx}") >= base_expr / (
                                    getattr(model, f"T{link_idx}") + new_ddl))))
                    base_r += fr
                    base_ddl = new_ddl
        # Solve the model.
        try:
